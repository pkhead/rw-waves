...
class SpawnSettingsInterface : PositionedMenuObject
{
    Menu.Remix.MenuTabWrapper tabWrapper;
    Menu.Remix.MixedUI.OpScrollBox scrollBox;

    Configurable<string> presetConfig;
    FileSystemWatcher fsWatcher;
    OpLabelLong descLabel;
    OpComboBox comboBox;
        ...
        : base(menu, owner, new Vector2(440f, 385f))
    {
        ...
        var config = new Configurable<string>(WaveSpawnData.SelectedPreset, new ConfigurableInfo(
        ...
        config.OnChange += () =>
        {
            ...
            WaveSpawnData.SelectedPreset = config.Value;
            descLabel.text = WaveSpawnData.Read().Description;
        };
        comboBox.OnChange += () =>
        {
            WavesMod.Instance.logger.LogInfo("selected preset changed to " + comboBox.value);
            WaveSpawnData.SelectedPreset = comboBox.value;
            descLabel.text = WaveSpawnData.Read().Description;
        };
        ...
        var descText = WaveSpawnData.Read().Description;
        yPos -= lineHeight;
    ...
    public override void GrafUpdate(float timeStacker)
    {
        ...
        // if (comboBox._rectList is not null)
        // {
        //     comboBox._rectList.fillAlpha = 0.1f;
        // }
    }
